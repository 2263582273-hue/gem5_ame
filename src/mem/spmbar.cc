...
SpmXBar::recvTimingReq(PacketPtr pkt, PortID slave_port_id)
{
    ...
    bool success = memSidePorts[master_port_id]->sendTimingReq(pkt);
...
{
    // determine the source port based on the id
    auto  *src_port = memSidePorts[master_port_id];
    ...
    bool success M5_VAR_USED = cpuSidePorts[slave_port_id]->sendTimingResp(pkt);
